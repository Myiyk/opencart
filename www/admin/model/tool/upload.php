...
class ModelToolUpload extends Model {
	...
	public function deleteUpload($upload_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "www WHERE upload_id = '" . (int)$upload_id . "'");
	}

	public function getUpload($upload_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "www` WHERE upload_id = '" . (int)$upload_id . "'");
	...
	public function getTotalUploads() {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "www";
